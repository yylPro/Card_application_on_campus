...
Page({
  data: { mode: 'login', phone: '', password: '', confirmPassword: '', staffName: '', gridName: '', operatorScope: '', accountGrid: '', gridOptions: [], selectedGrid: '全部网格', totalOrders: 0, rememberPassword: false, autoLogin: false, accountId: '', loggedIn: false, globalAddress: '', dateFrom: '', dateTo: '', exportKinds: ['activated'], exportActivatedChecked: true, exportPendingChecked: false, orders: [] },
  onLoad() {
  ...
  changeExportKind(event) {
    const values = event.detail.value || [];
    this.setData({ exportKinds: values, exportActivatedChecked: values.indexOf('activated') >= 0, exportPendingChecked: values.indexOf('pending') >= 0 });
  },
  ...
  async exportOrders() {
    const kinds = this.data.exportKinds || [];
    if (!kinds.length) return wx.showToast({ title: '请至少选择一种激活状态', icon: 'none' });
    try {
      ...
      const path = kinds.length === 1 ? (kinds[0] === 'activated' ? '/api/operator/export-activated' : '/api/operator/export-number-pending') : '/api/operator/export-selected';
      const result = await request(path, 'POST', { accountId: this.data.accountId, gridName: requestedGrid, from: this.data.dateFrom, to: this.data.dateTo, kinds });
