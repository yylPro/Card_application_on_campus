...
Page({
  data: { phone: "", password: "", confirmPassword: "", authMode: "login", staffSession: "", loggedIn: false, numberText: "", addressText: "", orders: [], loading: false },
  ...
  async login() {
    ...
    try { const result = await callCampus(data.authMode === "register" ? "campusStaffRegister" : "campusStaffLogin", { phone: data.phone, password: data.password, confirmPassword: data.confirmPassword, role: "operator" }); this.setData({ password: "", confirmPassword: "", staffSession: result.staffSession, loggedIn: true }); this.loadOrders(); } catch (error) { wx.showToast({ title: error.message, icon: "none" }); }
  },
  logout() { this.setData({ phone: "", password: "", confirmPassword: "", staffSession: "", loggedIn: false, orders: [] }); },
  parseNumbers() {
  ...
  async importNumbers() {
    ...
    try { const result = await callCampus("campusImportNumbers", { staffSession: this.data.staffSession, rows }); wx.showModal({ title: "导入完成", content: `成功 ${result.imported} 条，重复 ${result.skipped} 条${result.errors.length ? `，错误 ${result.errors.length} 条` : ""}`, showCancel: false }); this.setData({ numberText: "" }); } catch (error) { wx.showToast({ title: error.message, icon: "none" }); }
  },
  async loadOrders() {
    try { const result = await callCampus("campusListOrders", { staffSession: this.data.staffSession }); this.setData({ orders: result.orders || [] }); } catch (error) { wx.showToast({ title: error.message, icon: "none" }); }
  },
  ...
  async assignAddress(event) {
    ...
    try { await callCampus("campusAssignOutlet", { staffSession: this.data.staffSession, orderId: order.id, outletAddress: order.draftAddress }); wx.showToast({ title: "地址已更新", icon: "success" }); this.loadOrders(); } catch (error) { wx.showToast({ title: error.message, icon: "none" }); }
  },
});
