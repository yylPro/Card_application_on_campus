...
Page({
  ...
  async login() {
    ...
    try { await callCampus("campusStaffLogin", { staffKey: this.data.staffKey, role: "outlet" }); this.setData({ loggedIn: true }); } catch (error) { wx.showToast({ title: error.message, icon: "none" }); }
  },
  parseMessages() {
  ...
  async importMessages() {
    ...
    try { const result = await callCampus("campusImportVerification", { staffKey: this.data.staffKey, role: "outlet", messages }); this.setData({ results: result.results || [], messageText: "" }); } catch (error) { wx.showToast({ title: error.message, icon: "none" }); }
  },
});
