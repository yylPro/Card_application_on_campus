...
- 学生 H5：`http://127.0.0.1:4173/service/TEST-2026`
- 运营商后台登录：`http://127.0.0.1:4173/admin/login`
- Excel 导出：后台登录后访问 `http://127.0.0.1:4173/api/admin/export.xlsx`

二维码可以指向学生 H5 或跳转页；同一个二维码入口中，学生通过学校搜索和学院选择完成分流。H5 和小程序共用同一个后端接口与号码库存，但任一端不依赖另一端运行。
...
首次启动后，打开学生入口或运营商后台。开发环境的后台账号由 `.env` 中的 `ADMIN_USER`、`ADMIN_PASSWORD` 控制，公开部署前必须替换为独立的强密码。
...
- 运营商后台可下载 `/api/admin/export.xlsx`，包含已提交的学生和选号信息，便于线下核验与汇总。
